<?php
	...
	class DatabaseObject_Helper_ProductDisplay extends DatabaseObject
	{
		...
		public static function fetchVEproducts($db){
			$select = $db->select();
			$select->from(array('p' => 'products'), '*');
			$select->join(array('i' => 'product_inventories'),
                    				'p.product_id = i.product_id');
			$select->where('p.product_tag =?', 'Pants');
			//$select->group('i.product_id');
			
			echo $select;
			
			$productTmp = $db->fetchAll($select);
			$product;
			foreach($productTmp as $k=>$v){
				
					if($v['sys_quantity']==0){
						$product[$v['name']]['size'][$v['sys_bottom_size']]='None in stock';
					}else{
						$product[$v['name']]['size'][$v['sys_bottom_size']]=$v['sys_quantity'];
					}
