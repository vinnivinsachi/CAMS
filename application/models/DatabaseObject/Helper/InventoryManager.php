<?php
	...
	class DatabaseObject_Helper_InventoryManager extends DatabaseObject
	{
		...
		public static function addsimpleinventory($db, $inventory){
			$data = $inventory;
			
			$db->insert('simple_inventory', $data);
		}
		
		public static function retrieveAllInventory($db, $params=array()){
			$select=$db->select();
			$select->from('simple_inventory', '*');
			
			
			if(isset($params['style'])){
			$select->where('style=?',$params['style']);	
			}
			if(isset($params['tag'])){
			$select->where('tag=?',$params['tag']);
			}
		...
		public static function removeSimpleInventory($db, $param=array()){
			$db->delete('simple_inventory', array("id = '".$param['id']."'"));
