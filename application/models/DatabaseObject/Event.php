<?php
	...
	class DatabaseObject_Event extends DatabaseObject
	{
		...
		{
			$this->url= DatabaseObject_StaticUtility::generateUniqueUrl($this->_db, $this->_table, $this->profile->name, $this->uploader_id);
			return true;
		}
		...
		public static function GetObjects($db, $options=array()) //got the user, got form, got to, got order. 
		{
			...
			if(!empty($options['alphabetLink']) && strlen($options['alphabetLink'])>0)
			{
				...
				$select->where('pp.profile_key = "promotion_code"');
		...
		public static function GetTagSummary($db, $uploader_id)
		{
			//echo "your uploader_id is: ".$uploader_id;
			return DatabaseObject_StaticUtility::getTagSummaryForObject($db, $uploader_id, 'events_tags','', 'events', self::EVENT_STATUS_LIVE, 'event_id');
		}
		...
		public function eventTestLoad($ID)
		{
			...
			$select->join(array('p' => 'user_promotion'),
                    				'p.event_id = e.event_id')
			
			->where('p.event_id = ? ', $ID)
			->where('e.status != ?', 'NA')
			->where('p.promotion_application_status=?', 'Completed');
			//echo $select;
		...
		public function loadLiveObject($uploader_id, $url)
		{
			
		
			$query = DatabaseObject_StaticUtility::loadLiveObjects($this->_db, $uploader_id, $url, $this->_table, $this->getSelectFields(), self::EVENT_STATUS_LIVE);
