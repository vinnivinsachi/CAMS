	...
	class FormProcessor_Event extends FormProcessor
	{
		...
		public function __construct($db, $userID, $event_id=0)
		{
			...
			if($this->event->isSaved())
			{
				...
				$this->promotion_code =$this->event->profile->promotion_code;
			...
			else
			{
				...
				$this->event->uploader_id = $this->user->getId(); //$this->user_id at blogPost object came from this. 
			}
		}
		...
		public function process(Zend_Controller_Request_Abstract $request)
		{
			...
			$this->code = $this->sanitize($request->getPost('promotion_code'));
			if(strlen($this->code)==0){
			$this->addError('promotion_code', 'Please enter a code for this promotion');	
			}
			...
			$this->ticket_percentage = $this->sanitize(trim(($request->getPost('ticket_percentage'))));

			
			//echo "<br/>your ticket_price is: ".$this->ticket_price;
			if($this->ticket_price == 'FREE' || $this->ticket_price =='')
			{
			...
			if(!$this->hasError())
			{
				...
				$this->event->profile->promotion_code = $this->code;
				$this->event->profile->price = $this->ticket_price;
				$this->event->profile->percentage=$this->ticket_percentage;
