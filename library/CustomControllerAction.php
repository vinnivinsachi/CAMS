<?php
	...
	class CustomControllerAction extends Zend_Controller_Action //this can set post dispatch items. for header.tpl and footer.tpl
	{
		...
		public function init(){
			...
			$this->guestShippingInfo = new Zend_Session_Namespace('guestShippingInfo');
			...
			if($this->getRequest()->getParam('language')=='CH' && $_SESSION['language']=='EN'){
				$_SESSION['language']='CH';
			}elseif($this->getRequest()->getParam('language')=='EN' && $_SESSION['language']=='CH'){
			...
			$this->view->currentLanguage = $_SESSION['language'];

			//Zend_Debug::dump( $_SERVER['REQUEST_URI']);
			
			if(stripos($_SERVER['REQUEST_URI'], '?')===false){
				$this->view->chinese = $_SERVER['REQUEST_URI'].'?language=CH';
				$this->view->english = $_SERVER['REQUEST_URI'].'?language=EN';
			}else{
				$this->view->chinese = $_SERVER['REQUEST_URI'].'&language=CH';
				$this->view->english = $_SERVER['REQUEST_URI'].'&language=EN';
			}
		...
		public function preDispatch(){
			...
			if(!isset($this->shoppingCartInfoSession->cartInfo->invoiceNumber)){
			...
			if(!isset($this->shoppingCartInfoSession->cartInfo->promotionTypeUsed)){
			...
			$this->getResponse()->setHeader('content-type','text/html; charset=gb2312');

			
			//Zend_Debug::dump($this->guestShippingInfo);
