...
function togglePostFromTag(div){
		...
		tagToggleSections = $$('.subCategoryHeader');
		tagToggleSections.each(function(value){
			//					   alert('hiding stuff');
				value.removeClassName('categoryActive');	 
		 });
		
		var currentToggle = div.down();
		div.addClassName('categoryActive');
		var idString = currentToggle.id;
		var tagID = idString.split("-");
		//alert(tagID[1]);
		
		postSections = $$('.postTagSections');
		postSections.each(function(value){
			//					   alert('hiding stuff');
				value.addClassName('hidden');	 
		 });
		
		//alert('at showing how');
		var postTagShow = $('postTagID-'+tagID[1]);
		postTagShow.removeClassName('hidden');
}